...
def bookings(request):
    ...
    if request.method == 'POST':
        ...
        transaction_id_r = request.POST.get('itransaction_id')
        transaction_image_r = request.FILES.get(
            'itransaction_image')  # Get the uploaded image
        ...
        if bus:
            if bus.rem > int(seats_r):
                ...
                filename = fs.save(transaction_image_r.name,
                                   transaction_image_r)
                uploaded_image_url = fs.url(filename)

                book = Book.objects.create(name=username_r, email=email_r, userid=userid_r, bus_name=name_r,
                                           source=source_r, busid=id_r,
                                           dest=dest_r, price=price_r, nos=seats_r, date=date_r, time=time_r, transaction_id=transaction_id_r,
                                           transaction_image=uploaded_image_url, status='BOOKED')

                print('------------book id-----------', book.id)
                return render(request, 'myapp/bookings.html', locals())
            else:
                context["error"] = "Sorry, select a fewer number of seats"
                return render(request, 'myapp/findbus.html', context)
...
def cancellings(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect('myapp:seebookings')
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        profile_picture_r = request.FILES.get('profile_picture')

        user = User.objects.create_user(name_r, email_r, password_r)
        if user:
            if profile_picture_r:
                user.profile_picture = profile_picture_r
                user.save()
